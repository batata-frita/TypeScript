namespace ts {
    ...
    export function resolveTypeReferenceDirective(typeReferenceDirectiveName: string, containingFile: string | undefined, options: CompilerOptions, host: ModuleResolutionHost): ResolvedTypeReferenceDirectiveWithFailedLookupLocations {
            ...
            if (typeRoots && typeRoots.length) {
                ...
                return forEach(typeRoots, typeRoot => {
                    ...
                    return resolvedTypeScriptOnly(
                        loadNodeModuleFromDirectory(getFileExtension(containingFile), Extensions.DtsOnly, candidate,
        ...
        function secondaryLookup(): PathAndPackageId | undefined {
            ...
            if (initialLocationForSecondaryLookup !== undefined) {
                ...
                const result = loadModuleFromNearestNodeModulesDirectory(Extension.Ts, Extensions.DtsOnly, typeReferenceDirectiveName, initialLocationForSecondaryLookup, moduleResolutionState, /*cache*/ undefined);
    ...
    function tryLoadModuleUsingBaseUrl(parentFileExtension: Extension, extensions: Extensions, moduleName: string, loader: ResolutionKindSpecificLoader, state: ModuleResolutionState): Resolved | undefined {
        ...
        if (paths) {
            ...
            const resolved = tryLoadModuleUsingPaths(parentFileExtension, extensions, moduleName, baseUrl, paths, loader, /*onlyRecordFailures*/ false, state);
            if (resolved) {
    ...
    function tryResolveJSModuleWorker(moduleName: string, initialDir: string, host: ModuleResolutionHost): ResolvedModuleWithFailedLookupLocations {
        return nodeModuleNameResolverWorker(Extension.Ts, moduleName, initialDir, { moduleResolution: ModuleResolutionKind.NodeJs, allowJs: true }, host, /*cache*/ undefined, /*jsOnly*/ true);
    }

    export function nodeModuleNameResolver(moduleName: string, containingFile: string, compilerOptions: CompilerOptions, host: ModuleResolutionHost, cache?: ModuleResolutionCache): ResolvedModuleWithFailedLookupLocations {
        return nodeModuleNameResolverWorker(getFileExtension(containingFile), moduleName, getDirectoryPath(containingFile), compilerOptions, host, cache, /*jsOnly*/ false);
    }

    function nodeModuleNameResolverWorker(parentFileExtension: Extension, moduleName: string, containingDirectory: string, compilerOptions: CompilerOptions, host: ModuleResolutionHost, cache: ModuleResolutionCache | undefined, jsOnly: boolean): ResolvedModuleWithFailedLookupLocations {
        const traceEnabled = isTraceEnabled(compilerOptions, host);
        ...
        function tryResolve(extensions: Extensions): SearchResult<{ resolved: Resolved, isExternalLibraryImport: boolean }> {
            ...
            if (!isExternalModuleNameRelative(moduleName)) {
                ...
                const resolved = loadModuleFromNearestNodeModulesDirectory(parentFileExtension, extensions, moduleName, containingDirectory, state, cache);
    ...
    function loadModuleFromFile(parentFileExtension: Extension, extensions: Extensions, candidate: string, onlyRecordFailures: boolean, state: ModuleResolutionState): PathAndExtension | undefined {
        if (extensions === Extensions.Json) {
            ...
            return extensionLess === undefined ? undefined : tryAddingExtensions(parentFileExtension, extensionLess, extensions, onlyRecordFailures, state);
        }
    ...
    function loadNodeModuleFromDirectoryWorker(parentFileExtension: Extension, extensions: Extensions, candidate: string, onlyRecordFailures: boolean, state: ModuleResolutionState, packageJsonContent: PackageJsonPathFields | undefined, versionPaths: VersionPaths | undefined): PathAndExtension | undefined {
        const fromPackageJson = packageJsonContent && loadModuleFromPackageJson(packageJsonContent, versionPaths, parentFileExtension, extensions, candidate, state);
        if (fromPackageJson) {
            return fromPackageJson;
        }
        const directoryExists = !onlyRecordFailures && directoryProbablyExists(candidate, state.host);
        return loadModuleFromFile(parentFileExtension, extensions, combinePaths(candidate, "index"), !directoryExists, state);
    }
    ...
    function loadModuleFromPackageJson(jsonContent: PackageJsonPathFields, versionPaths: VersionPaths | undefined, parentFileExtension: Extension, extensions: Extensions, candidate: string, state: ModuleResolutionState): PathAndExtension | undefined {
        ...
        if (versionPaths && containsPath(candidate, file)) {
            ...
            const result = tryLoadModuleUsingPaths(parentFileExtension, extensions, moduleName, candidate, versionPaths.paths, loader, onlyRecordFailures, state);
            if (result) {
        ...
        return removeIgnoredPackageId(loader(parentFileExtension, extensions, file, onlyRecordFailures, state));
    }
    ...
    function loadModuleFromNearestNodeModulesDirectory(parentFileExtension: Extension, extensions: Extensions, moduleName: string, directory: string, state: ModuleResolutionState, cache: NonRelativeModuleNameResolutionCache | undefined): SearchResult<Resolved> {
        return loadModuleFromNearestNodeModulesDirectoryWorker(extensions, moduleName, directory, state, /*typesScopeOnly*/ false, cache, parentFileExtension);
    }
    ...
    function loadModuleFromNearestNodeModulesDirectoryTypesScope(moduleName: string, directory: string, state: ModuleResolutionState): SearchResult<Resolved> {
        ...
        return loadModuleFromNearestNodeModulesDirectoryWorker(Extensions.DtsOnly, moduleName, directory, state, /*typesScopeOnly*/ true, /*cache*/ undefined, Extension.Ts);
    }

    function loadModuleFromNearestNodeModulesDirectoryWorker(extensions: Extensions, moduleName: string, directory: string, state: ModuleResolutionState, typesScopeOnly: boolean, cache: NonRelativeModuleNameResolutionCache | undefined, parentFileExtension: Extension): SearchResult<Resolved> {
        ...
        return forEachAncestorDirectory(normalizeSlashes(directory), ancestorDirectory => {
            if (getBaseFileName(ancestorDirectory) !== "node_modules") {
                ...
                return toSearchResult(loadModuleFromImmediateNodeModulesDirectory(extensions, moduleName, ancestorDirectory, state, typesScopeOnly, parentFileExtension));
            }
        });
    }

    function loadModuleFromImmediateNodeModulesDirectory(extensions: Extensions, moduleName: string, directory: string, state: ModuleResolutionState, typesScopeOnly: boolean, parentFileExtension: Extension): Resolved | undefined {
        ...
        const packageResult = typesScopeOnly ? undefined : loadModuleFromSpecificNodeModulesDirectory(extensions, moduleName, nodeModulesFolder, nodeModulesFolderExists, state, parentFileExtension);
        if (packageResult) {
        ...
        if (extensions !== Extensions.JavaScript && extensions !== Extensions.Json) {
            ...
            return loadModuleFromSpecificNodeModulesDirectory(Extensions.DtsOnly, mangleScopedPackageNameWithTrace(moduleName, state), nodeModulesAtTypes, nodeModulesAtTypesExists, state, parentFileExtension);
        }
    }

    function loadModuleFromSpecificNodeModulesDirectory(extensions: Extensions, moduleName: string, nodeModulesDirectory: string, nodeModulesDirectoryExists: boolean, state: ModuleResolutionState, parentFileExtension: Extension): Resolved | undefined {
    ...
    export function loadModuleFromGlobalCache(moduleName: string, projectName: string | undefined, compilerOptions: CompilerOptions, host: ModuleResolutionHost, globalCache: string): ResolvedModuleWithFailedLookupLocations {
        ...
        const failedLookupLocations: string[] = [];
        const state: ModuleResolutionState = { compilerOptions, host, traceEnabled, failedLookupLocations };
        const resolved = loadModuleFromImmediateNodeModulesDirectory(Extensions.DtsOnly, moduleName, globalCache, state, /*typesScopeOnly*/ false, Extension.Ts);
